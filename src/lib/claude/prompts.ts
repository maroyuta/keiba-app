...
- **過去走の頭数(entry_count)が少ないほど、その内容を額面通りに高評価しないこと。** 18頭前後のフルゲートを基準に、頭数が1頭減るごとに連続的・段階的に割り引くイメージで考える(9頭・13頭・14頭のように「小頭数」に見えない頭数でも、フルゲートとの差の分だけ同じ理屈が働く)。固定の頭数しきい値で機械的に判定するのではなく、そのレースの頭数・展開・実際の着差から「本当に力を要する内容だったか」を都度見極めること。**ただしメンバーレベルが高い(格上挑戦馬が複数出走、上位人気馬の実績が濃い等)場合は話が別で、頭数の少なさだけを理由に一律で割り引かないこと** — 割り引くべきは「頭数が少なく、かつ相手のレベルも低かったために楽に走れた」ケースであり、少頭数でも実際に力のある相手と競り合った内容なら額面通り評価してよい。特にfinish_position/margin_secが欠損している(データが薄い)過去走を「悪くなさそう」と好意的に解釈するのは禁物で、判断材料が乏しいこと自体を明確にマイナス材料として扱うこと(データが無い=中立ではなく、信頼度を下げる要因)。多頭数・きわどい競り合いの中での実績の方を優先的に評価すること
...
- **★古いコース実績で直近の凡走を正当化しないこと(2026-07-18、実際の外れレースで確認)。** 「その馬は
  ...
  優先すること
- **「コース実績(このコースで好走したことがある)」「距離初挑戦・初距離」を、選定理由の主役にしすぎないこと。**
  上記の「古い実績で直近凡走を正当化」ほど極端でなくても、これらは補足的な参考情報にとどめ、直近の
  competitive(僅差・多頭数・格上手応え等)な実際の走りの内容を主軸に評価すること。「初距離だが血統・
  走破スタイルから距離延長は合いそう」のような未実証の期待だけで高評価にせず、参考にする場合も他の
  具体的な根拠(直近の着差・位置取り・脚色)と併記して初めて理由として成立させること。完全に無視して
  よいわけではない(コース適性・距離適性自体は実在する要素)が、単独の決め手にしないこと
