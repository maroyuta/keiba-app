...
- 本命(軸)は複勝率を最優先で選び、価格(オッズ)"だけ"を理由に上位人気から動かさない(下記『本命=守り』が優先。
  ...
  **★本命(守り)を"価格の綾"だけで1番人気から動かさない(2026-08-09。実測で本命が1番人気ベタ買いに負けている
  主因が、弱い根拠での微妙な乗り換えだった)。ただし機械的に1番人気を軸にするのは素人の予想であり禁止——
  本命の技術は「上位人気(1〜3番人気)の中から"最も堅い1頭"を、能力×トラックバイアス×レース内の不利の3軸で
  見抜く」ことにある。** 1番人気を既定の錨としつつ、各過去走のbias_noteの価値判定(『この好走はバイアス恩恵込み
  ＝額面ほどの価値なし』『この大敗は不利で沈んだ＝度外視・上方修正』)を使って:
  - 1番人気の近走好走がバイアス・展開頼みのフロークで、かつ2〜3番人気に「不利を受けても崩れず走った/
    バイアス中立〜逆行で堅実」な"地力が上位"の馬がいるなら、そちらを本命にしてよい(これは価格でなく3軸の事実で動かす)。
  - 逆に1番人気が地力・展開・バイアスのどれで見ても堅いなら、素直に1番人気を本命にする(無理に穴へ動かさない)。
  価格や漠然とした妙味だけで上位人気から動かすのは禁止だが、3軸の強い事実があれば1〜3番人気の中で最も堅い馬へ動かす——
  これが"脳死1番人気"でも"弱い根拠での逆張り"でもない、守りの本命選定である。
...
- **本命(守り): 目的は複勝率の最大化。市場に錨を下ろした保守的な選定。** 原則、最も堅く複勝に来る市場上位馬
  (1〜2番人気を中心)を軸にする。データ(バイアス・ペース・能力)は"隠れた穴を見つける攻め"ではなく、
  **"危ない人気馬を避ける守り"に使う。** 次のいずれかが立った時だけ、その人気馬を軸から外して次の堅い馬へ下げる:
  (a)その馬の近走好走が実はペース・バイアスに助けられただけ(=bias_noteの「バイアス有利だった中での結果」や
  ペース注記の「スロー起因」に該当)、(b)今回致命的なバイアス不利を受ける枠・脚質、(c)昇級・距離延長など
  未知条件で近走を額面通り信用できない。**上記が無ければ、妙味を理由に本命を上位人気から動かさない**
  (妙味は相手でやる)。バイアス比重アップ(バイアス>戦績)は主に相手選定に効かせ、本命は市場に錨を下ろす。
  - **★ダートは特に市場最上位に錨を下ろすこと(2026-08-09データで確定)。** 直近25レースで本命複勝率は
    ダート36%(5/14)・芝64%(7/11)と、ダートで本命が明確に飛んでいる。ダートはゲート・ペース・馬場依存が
    大きく、本命を1番人気から動かした逆張りが芝以上に外れやすい。**ダートでは上の(a)(b)(c)の脆さ根拠が
    "特に明確"な時だけ動かし、原則1番人気(=市場最上位)を素直に軸にすること。**
...
「1番人気ベタ買い」64%(16/25)を16ポイント下回り、特にダートは36%(5/14)まで落ちていた。傾向は継続しており、
本命は1番人気への錨をさらに強める方向で運用すること。** つまり現状のロジックは、バイアス・展開等の調整を加えたことで、単純に人気上位馬を
