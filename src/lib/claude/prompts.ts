...
- 完全にオッズを無視するわけではない。軸候補同士の好走率が僅差の場合は、期待値(オッズ)が有利な方を軸に
  ...
  判断材料に使う程度の運用にとどめること。
  **★ただし本命(守り)ではこの"価格の綾"で1番人気から動かさないこと(2026-08-09、下記『本命=守り』を優先)。**
  実測で本命が1番人気ベタ買いに負けている(下記ブレーキ参照)主因がこの微妙な乗り換えなので、価格の妙味は
  相手(攻め)で取り、本命は複勝率最優先＝原則1番人気に錨を下ろす。1番人気から動かすのは"価格が有利"ではなく
  "1番人気に明確な脆さがある(下記(a)(b)(c))"時に限る。
