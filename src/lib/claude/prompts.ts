...
- 各レース最低2頭にhorse_rank="S"を付与する。妙味が全体的に薄いレースでも2頭は選出すること
- 妙味の水準が僅差で並ぶ馬が3頭以上いる場合、無理に2頭に絞り込まず、該当する馬は全てSにしてよい
  (同列の妙味候補が複数いるのは自然なことであり、機械的に上位2頭だけに絞る必要はない)
- ただし単なる相対順位での機械的選出ではなく、絶対水準でも「実際に買うに値する馬」であることを条件とする
