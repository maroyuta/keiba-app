...
- **脚質による「見た目の強さ」のバイアスに注意すること(2026-07-13、実データのバックテストで確認)。**
  差し脚質の馬は展開に恵まれて上がり3F(終いの脚)だけが目立って速く見えることがあり、その上がりタイム
  だけで実力を過大評価しないこと。差しは想定ペースが噛み合わないと再現性が低い(前が残るペースだと
  届かない)。**逆に先行して粘るタイプは、派手な上がりタイムが出にくいために見た目の評価が伸びにくく、
  実際の実力比では過小評価されがちで妙味が出やすい傾向がある。** 差しを機械的に排除するわけではないが、
  通過順位・上がりタイムだけでなく、想定ペースとの適性(展開が向くかどうか)まで含めて判断すること
