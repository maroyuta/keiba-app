...
- **脚質は「絶対能力」ではなく「その日の展開が有利だったか不利だったか」との相対関係で評価すること
  ...
  - **★ブリンカー「解除」は明確な減点材料にすること(2026-08-06、race_entries実測で確定)。**
    ...
    突出して悪いシグナルである点に注意
  - **★穴馬(概ね10番人気以下)の休み明けはむしろ評価を上げる材料にすること(2026-08-06、race_entries
    実測で確定)。** 前走からの間隔(pastPerformances[0]の race_date と今回の race.race_date の差)が
    120日以上の「休み明け」馬について、人気帯別の単勝回収率を実測したところ、1〜9番人気では休み明けと
    通常出走で回収率に有意差が無かった(市場が正しく織り込み済み)一方、**10番人気以下の穴馬に限っては
    休み明けの方が通常出走より単勝回収率が明確に高かった(81.8%対60.0%、n=773対12,226)**。上の
    「軸馬評価の参考ヒント」にある"休み明け・久々のレースで過去の休み明け成績が悪い"は軸(honmei)候補
    である上位人気馬の話であり矛盾しない——**この新ルールは穴馬(aite/S候補で10番人気以下)側にのみ
    適用し、休み明けであること自体を理由に評価を下げない。** むしろ市場が休み明けの穴馬を十分に
    割り引けていない(過小評価している)ことの表れとして、妙味判断でプラス材料に使うこと。なお
    厩舎別の休み明け傾向(この厩舎は休み明けに弱い/強い等)は現状のサンプル数(n=15〜20程度)では
    ノイズと区別できず未確定のため、個別厩舎名では判断しないこと
...
- **昇級戦、および昇級後2戦目までの馬はhonmei/aiteどちらにも採用しないこと(2026-08-02、
  ユーザー指摘により改定)。** 昇級戦は「同じ距離・条件」に見えても相手のレベルが実質的に別物であり、
  ...
  は禁止。
  - **honmei/aiteどちらも対象。** 以前は「honmeiは圧倒的な内容があれば例外」としていたが、
    実際にその例外運用が機能せず(昇級初戦・2戦目の馬が繰り返し買われる事故が発生)、ユーザーから
    「メンバーレベルが格段に変わるのだから例外なく購入を控えたい」と明確な指摘があったため、
    例外を撤回した。今回のクラスでの実績が0走(初戦)または1走(2戦目)の馬は、絶対能力がどれだけ
    高く見えても honmei_horse_number/aite_horse_number/aite_horse_number_2 のいずれにも採用しない
    (格上げの段差が大きいほど通用力の不確実性が増す点も踏まえると、なおさら例外を設ける理由は薄い。
    実例: カテリーナ=3勝クラスからオープンを飛ばして重賞挑戦し5着、カウンターセブン=同様の重賞
    格上げ。下のクラスでの好タイムはメンバー構成・ペース質が異なるため上のクラスでの通用力の根拠に
    ほとんどならない)
  - **この判定は診断コード側でも機械的にチェックしており(enforceClassUpGuard)、違反した場合は
    書き込み前に自動で見送りへ差し戻される。** プロンプト側で外し忘れても購入自体は止まる設計だが、
    無駄な機械チェック依存にしないため、診断の時点で自分自身が正しく外すこと
