...
- **★古いコース実績で直近の凡走を正当化しないこと(2026-07-18、実際の外れレースで確認)。** 「その馬は
  ...
  優先すること
- **「コース実績」「距離適性・初距離」は実在する評価軸であり、両極端を避けること。** 過大評価(古い/一度きりの
  好走実績だけで直近の凡走や不安要素を覆い隠す)と、過小評価(反射的に「参考程度」へ格下げし、実質的に
  評価から締め出す)のどちらにも寄らないこと。直近の実際の走りと矛盾しない、あるいはそれを補強する形で
  コース実績・距離適性が示されているなら、それ単独でも十分に有効な根拠として通常通り評価してよい
  (「そのデータがあるなら買い」で構わない場面は普通にある)。避けるべきなのは、①直近の凡走・不安要素を
  古い好走実績「だけ」で正当化すること、②逆に根拠薄弱な期待(「初距離だが血統的に合いそう」等、他の
  裏付けが無い当てずっぽう)を距離適性の名目で高評価にすること。いずれの場合も、直近の実際の走りの内容
  (着差・位置取り・脚色)と矛盾していないかを必ず確認した上で判断すること
...
- **昇級戦(今回のクラスで初めて走る馬)はhonmei/aite候補にしない(2026-08-02、ユーザー指摘)。**
  昇級戦は「同じ距離・条件」に見えても相手のレベルが実質的に別物であり、下のクラスでの実績(勝ち鞍を
  含む)は今回のクラスでの通用力を保証しない。特に「下のクラスでの古い勝ち鞍」を根拠に、直近の
  凡走・大敗を大目に見て相手に採用するような判断(実例: 新潟6Rでショウサンジョージ=7ヶ月前の
  下級条件での勝利を根拠に、直近10着0.8差を「崩れなかった」と好意的に解釈して相手採用)は禁止。
  昇級初戦の馬は、今回と同等以上のクラスで実際に通用した実績が別途無い限りhonmei/aiteから外し、
  horse_rank・horse_rank_commentでは「昇級初戦のため見送り」等、判断材料として通常通り言及してよい
  (診断自体は行うが、購入対象からは外す)
