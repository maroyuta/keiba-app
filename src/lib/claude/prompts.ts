...
- **★開幕週(is_opening_week=true、nichiji<=2)は要注意。** 同じ競馬場でも、開催後半(nichiji大)に
  ...
  - **★新潟は「開幕週=内前有利」という単純な決め打ちをしないこと(2026-07-23、ネット複数ソース+
    ...
    - 外回り(1400〜1800m等)の内外は、ネット上の分析記事(note「新潟競馬場・夏 開幕週の傾向と対策」)は
      ...
      判断材料に留めること**
  - **★新潟ダート(レパードS等)は「内有利」より「前有利」の方が確度が高いシグナル(2026-08-04、
    自社DB実測)。** 今節(2026年7/25・26・8/1・8/2、新潟ダート1800m・1200m)を日別に集計すると、
    脚質は先行・逃げ勢が差し・追込勢より一貫して着順比が良く(1800mの8/1・8/2とも
    逃げ<先行<差し<追込の順で単調に悪化、方向が崩れていない)、これは中京・札幌ダート/芝でも
    同じ順序が出ている(下記の場横断メモ参照)。一方、枠(内外)は開催前半(7/25-26、稍重〜不良)は
    外枠有利、後半(8/1-8/2、稍重→良と回復)につれ内枠有利へ単調に転換しており、**今年に限れば
    「馬場が乾くほど内が良くなる」形で内前有利化が進んでいるが、これが新潟ダートの構造的な特性とは
    言い切れない**(2025年の同時期開催はほぼ良馬場を通した上で終始外枠有利、開催終盤(nichiji8)は
    むしろ内枠が明確に不利(平均着順比 内64% 対 外42%)で終わっており、「開催が進む=内が締まる」
    という単純な法則の裏付けにはならない)。レパードS等このコースの本命選定では、枠の内外より
    先行力・好位につけられるかを主要な判断材料にし、枠についてはbias_reference_races(今週土曜+
    先週同場)の実測とその週の馬場乾湿を必ず確認すること
  - **★2026年7月末〜8月頭の新潟・中京・札幌3場では、脚質面の「前有利」(逃げ・先行が差し・追込より
    優勢)が場・芝ダート問わず最も一貫して出ていたシグナルだった(2026-08-04実測、past_performancesの
    corner_positions由来)。** 札幌ダートが特に明瞭(平均着順比: 逃げ36%→先行50%→差し54%→追込63%、
    低いほど好走)。bias_reference_racesが薄い/内外の判定が割れる場合でも、「差し・追込は一段評価を
    下げる」は優先してよいデフォルト仮説として扱える
  - **★中京芝の「内枠先行有利」一般論(上記)は、2026年開催の今節(7/25-8/1)の実測ではむしろ外枠有利で
    推移していた(平均着順比: nichiji1内53%対外55%、nichiji2内50%対外58%、nichiji3内55%対外54%、
    8/2のnichiji4のみ内59%対外48%と反転、2026-08-04確認)。一般論をそのまま延長せず、
    bias_reference_racesの実測(直近の同場)を優先すること**
