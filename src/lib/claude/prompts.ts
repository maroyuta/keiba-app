...
- 勝率・的中率の高さそのものではなく、この妙味(エッジ)が最大の馬を優先してhonmei/aiteを選ぶこと
- **人気同士の組み合わせを人気だからという理由だけで機械的に除外しないこと。** 例えば1・2番人気が他を大きく突き放していて2頭とも掲示板に載る可能性が非常に高い場合、そのワイドの実際の払い戻し(オッズ)が4倍程度あるなら、combo自体の妙味(その組み合わせの的中確率に対して配当が見合っているか)で見て十分魅力的なことがある。判断基準は「人気同士かどうか」ではなく「その組み合わせの実際のワイド/馬連オッズが、想定的中確率に対して妙味があるか」
...
- 理想的な買い目は「掲示板に載る可能性が高い手堅い1頭」+「妙味(エッジ)のある1頭」の組み合わせ、または「実際のワイド/馬連オッズが的中確率に対して妙味のある人気同士の組み合わせ」のいずれか。単に人気薄同士を並べただけの組み合わせ(的中率が低すぎて安定しない)は避ける
- analysis_valueには「過小評価されている」で終わらせず、市場想定勝率と実力の差(可能なら組み合わせオッズとの比較)という上記の妙味の考え方に基づいた具体的な根拠を書くこと

## 軸(本命)とS評価(妙味候補)の役割分担 ★重要

honmei_horse_number(軸・本命候補)とhorse_rank="S"(妙味候補)は評価基準が別物であり、混同しないこと。

**軸(honmei_horse_number)の判断基準:**
- 絶対能力・着内率のみで選ぶ。軸にオッズ由来の妙味(エッジ)は要求しない
- 唯一のチェック項目は、近走に崩れリスク要因(不利・体重激変・展開不適合等)が隠れていないかどうか
- 1番人気だからと安易に固定しない。2番人気以下の対抗馬とも着内率・展開適性を必ず比較検討してから決めること
  (反省事例: 2026-07-12七夕賞で1番人気を機械的に軸にしたが7着に終わり、実際は2番人気が1着だった。
  人気順ではなく着内率・展開適性の比較で軸を決めること)

**妙味候補(horse_rank="S")の判断基準:**
- 判断基準は「自己推定好走率 − オッズ逆算確率」のエッジ(EV)
...
1番人気・2番人気が実力的に明確に抜けていて他馬との実力差が大きい(混戦ではない)レースは、
その2頭を組み合わせても実際のワイド/馬連オッズが的中確率に見合わない(妙味がない)場合に限り、
race_rankをA以下に留める。組み合わせオッズが的中確率に対して十分な妙味を持つ場合はSにしてよい
(単に「人気同士だから」という理由だけでSを避けない)。
