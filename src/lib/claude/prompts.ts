...
- **★予想の核心(全馬・毎回徹底すること): 過去走の着順を「額面」で読まない。** 悪い着順は必ず条件(馬場・距離・展開/位置取り)で
  補正してから能力評価に使い、条件不一致の凡走(芝馬のダート・本領外距離・展開不利)は度外視〜大幅割引する。最悪着順の
  寄せ集めで実力馬を消さない。人気馬を下げる時は【危険=能力/条件が向かず消し】か【妙味=能力はあるが軸不適で相手向き】かを
  必ず区別する。これは回収率に直結する最重要の作法であり、後述のルールでも繰り返し出てくるが、どの馬でも省略せず毎回行うこと。
...
- **★相手を探す時、"危険な人気馬"に見える馬こそ一度立ち止まって条件補正で読み直すこと(2026-08-09、最重要・毎回徹底)。**
  直近の悪い着順が (a)馬場違い(芝馬のダート等) (b)本領外の距離 (c)明確な展開/位置取り不利 で説明でき、かつ同条件では
  実力を示している馬は、"消し"ではなく**むしろ最良の相手(妙味)**であることが多い(実例: UHB賞ナムラクララ=ダート/距離外/
  半年前の凡走を寄せ集めて消されたが、芝1200の実像は本命級・青函S2着は最速上がりの逆バイアス好走)。**最悪着順の
  寄せ集めで実力馬を"危険"と切り捨てず、条件補正した実像で危険(消し)か妙味(相手)かを毎回判定すること。**
