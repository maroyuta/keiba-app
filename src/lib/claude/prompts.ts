...
- **昇級戦(今回のクラスで初めて走る馬)の扱いは、honmeiとaiteで厳しさを分けること(2026-08-02、
  ユーザー指摘)。** 昇級戦は「同じ距離・条件」に見えても相手のレベルが実質的に別物であり、下の
  クラスでの実績(勝ち鞍を含む)は今回のクラスでの通用力を保証しない。特に「下のクラスでの古い
  勝ち鞍」を根拠に、直近の凡走・大敗を大目に見て採用するような判断(実例: 新潟6Rでショウサンジョージ
  =7ヶ月前の下級条件での勝利を根拠に、直近10着0.8差を「崩れなかった」と好意的に解釈して相手採用)
  は禁止。
  - **aite: 原則除外。** 今回と同等以上のクラスで実際に通用した実績が別途無い限り、昇級初戦の馬は
    aite候補から外す(「相手ならあれだけど」= aiteは妙味探しの場であり、昇級戦特有の不確実性を
    冒してまで採用する必要は薄いため)
  - **honmei: 全面除外まではしない。** 「今回のクラスでも通用する」と言い切れる圧倒的な内容
    (今回の昇級戦そのものでの圧勝・大差勝ちや、直近の下級条件での勝ち方が着差・内容ともに
    次のクラスでも通用すると具体的に説明できるレベル)があれば、昇級初戦でもhonmeiに採用してよい。
    「昇級初戦だから一律に外す」という機械的な運用は本命についてはやり過ぎ(2026-08-02、ユーザー
    指摘)。ただし採用する場合はrace_rank_reason/horse_rank_commentに、なぜ昇級の不確実性を
    上回ると判断したかの具体的根拠を明記すること
  - どちらの場合も、単に「診断自体」は通常通り行い、horse_rank・horse_rank_commentへの言及は妨げない
  - **格上げの「段差」の大きさを区別すること(2026-08-02、カテリーナ=3勝クラスからいきなり重賞挑戦
    で5着、カウンターセブン=同様の重賞格上げ、という実例を踏まえたユーザー指摘)。** 1勝クラス→2勝
    クラスのような通常の1段階の昇級と、3勝クラス→オープンを飛ばして重賞挑戦のような複数段階の
    飛び級では、後者の方が通用力の不確実性が明確に大きい。段差が大きいほどhonmeiの例外条件
    (「今回のクラスでも通用すると言い切れる圧倒的な内容」)のハードルを上げること。**また、下の
    クラスでの好タイムは、上のクラスでの通用力の根拠としてほとんど参考にならない**(クラスが違えば
    メンバー構成・ペース質・実質的な負荷が異なり、単純なタイム比較は意味を持たない)。「下のクラスで
    速い時計を出している」だけを飛び級の裏付けにしないこと
