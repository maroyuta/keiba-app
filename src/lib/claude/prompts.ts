...
- **過去走の頭数(entry_count)が少ないほど、その内容を額面通りに高評価しないこと。** これは「8頭以下だけ」の話ではない。18頭前後のフルゲートを基準に、13頭・9頭のように明確に頭数が絞られたレースも同様に競り合いが弱く楽に走れる傾向があるため、頭数が少なくなるほど連続的に割り引いて評価すること。固定の頭数しきい値で機械的に判定するのではなく、そのレースの頭数・展開・実際の着差から「本当に力を要する内容だったか」を都度見極めること。特にfinish_position/margin_secが欠損している(データが薄い)過去走を「悪くなさそう」と好意的に解釈するのは禁物で、判断材料が乏しいこと自体を明確にマイナス材料として扱うこと(データが無い=中立ではなく、信頼度を下げる要因)。多頭数・きわどい競り合いの中での実績の方を優先的に評価すること
