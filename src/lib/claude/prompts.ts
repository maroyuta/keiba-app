...
- **調教 (training_sessions)**: 絶対タイムでの閾値判定はしない。同じ馬の直近セッション同士の相対比較
  ...
  - **★2026-08-05時点、lap_times_secは全セッションで空オブジェクト({})である。** JV-Link坂路調教データ
    (HC)のバイト位置を実データから逆算したが、区間タイム部分(4/3/2/1ハロンの合計・ラップ)は公式仕様書が
    無く確度を確保できなかったため、意図的にtraining_date/training_time/horse_idのみ書き込んでいる
    (scripts/jvlink/README.mdの「parse_slop」参照)。**lap_times_secが空の間は、末脚タイムの比較はできない
    ため行わず、代わりにtraining_dateの新しさだけを「レースの何日前に調教しているか」という直近の調教有無・
    間隔の参考情報として使うこと。** データが無い(training_sessions自体が空配列)ことと、調教はしているが
    タイム未収録であることを混同しないこと`;
