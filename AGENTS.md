...
- **未実装(次回以降):** 上記はテーブル設計のみで、(1) BLOD由来データの実投入(JV-Link接続後)、(2) past_performancesの蓄積からsire_stats/nick_statsのroi_win_pct等を実際に算出するバッチ/クエリ、(3) `prediction_criteria`経由でこれらの集計値を診断プロンプトに渡す配線、はまだどれも手付かず。現状`prompts.ts`のbuildRaceDataPayloadはtraining_sessions/sire_stats/nick_stats/horse_pedigreesのいずれも診断入力に含めていない(スキーマ上存在するだけで診断エンジンにはまだ接続されていない)
...
5. **新設マイグレーション(`20260710070000_redesign_pedigree_training_stats.sql`)の適用・型チェック** — このセッションにSupabase CLI/node環境が無く、マイグレーションの実適用(`supabase db push`等)も`tsc --noEmit`での`database.types.ts`整合性確認もできていない。次回、環境がある場所で両方確認すること
6. **training_sessions・sire_stats/nick_statsの集計ロジック実装** — テーブル設計(`horse_pedigrees`・再設計済み`training_sessions`・`roi_win_pct`付き`sire_stats`/`nick_stats`)は完了したが、(a) JV-Link接続後の実データ投入、(b) past_performances蓄積からのROI等の実集計バッチ、(c) これらを`prediction_criteria`経由で診断プロンプト(`prompts.ts`のbuildRaceDataPayload)に渡す配線、はまだ未着手。運用初期はデータが薄い制約もあるため、当面は「穴馬推奨」機能の精度は限定的になる前提で進める
