...
**⚠️「実際に儲かる予想ができているか(回収率の実証)」は体感0%、事実上未着手。** 診断ロジック・
回収率トラッキングの配管(`race_payouts`/`compute_recommendation_results.py`)は完成しているが、
**過去レースへのバックテスト、実運用での的中実績とも1件も実施していない。** 技術的な完成度とは
別軸で、これが「このアプリが本当に機能するか」を左右する最大の未検証項目。次にやるべきは
過去100レース程度のバックテスト(2026-07-11に計画済み、まだ未着手)。
