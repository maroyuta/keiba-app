...
4. **残る設計判断:** 「実際に買う」判断のためのOpus深掘りを自動エスカレーションではなく手動の「本気診断」ボタンに変えるか(既存の「再診断する」ボタンを流用/分離)。現行コードはstandardでS評価が出ると自動でpremiumへエスカレーションする実装のままなので、手動化するなら`route.ts`の該当箇所の変更が必要
