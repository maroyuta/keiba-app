...
3. **実際の購入結果とAI推奨の回収率比較(配当確定待ち)。** ユーザーが2026-08-02に実際にIPATで
   ...
   - 札幌11R北海道新聞杯クイーンS(id `c697640a-1205-4bdc-86b9-06431278bfad`): 実購入5-9(馬連1,500+
     ワイド10,000=11,500円) / AI推奨は9→3(ワイド4,500/馬連500)
   - 2026-08-02のnetkeiba My収支合計: 購入57,200円・払戻0円(全レース未確定、上記6レース以外にも
     購入あり)
   - ユーザーの希望: 実際の購入(点数がバラバラ)は5,000円/レース換算(2点以上買っているものは
     5,000円÷点数で正規化)に正規化して回収率を出す。AI推奨側は既に5,000円/レースぴったりなので
     正規化不要。配当が確定(`race_payouts`にデータが入る)次第、両方の回収率を計算して報告すること
