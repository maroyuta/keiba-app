...
5. **JV-Link接続 — ✅ 疎通確認完了 (2026-07-11)。** Windows PC側でClaude Codeを使い、実際にJV-Linkから生データの取得に成功した(RA=レース詳細37件、SE=馬毎レース情報492件、JG=1002件、合計1531件を取得・確認済み)。作業はこのリポジトリではなくWindows PC上の`C:\Users\maroy\OneDrive\デスクトップ\jvlink\`で別途行われており、このリポジトリの`scripts/jvlink/fetch_raw.py`(Mac側セッションで作成した雛形)とは別の実装になっている。Windows側での実装・デバッグ過程で判明した重要な知見:
   ...
   - **✅完了(2026-07-13): ユーザーから詳細な「AI仕様書」が届き、`src/lib/claude/prompts.ts`の診断プロンプトを大幅改訂した。** 主な変更点: (0)ルールは絶対ではなく総合判断の補助線という前提を明記、(2)軸(honmei)は絶対能力ではなく「実質的な複勝率(能力×枠バイアス適性×展開適性)」最大化基準に変更し、僅差時のみオッズをタイブレークに使ってよいとした。軸馬評価の参考ヒント(直前オッズ上昇・休み明け実績・ペース脚質ミスマッチ・格上げ初戦・馬体重急変・マイナス方向の乗り替わり)を追加、(2続き)S評価の「4〜9番人気帯」ガイドラインは機械的な足切りラインではなく目安である旨を明記、(3)極端な鉄板(1番人気1.5倍前後)・少頭数(10頭未満)レースの取り扱いルールを追加(前者はrace_rank=C、後者は診断はするが馬券対象外でhonmei等をnull化)、(4)1番人気×2番人気の組み合わせを明示的に禁止(以前は「組み合わせオッズ次第で許容」だった点を撤回)。**実際に七夕賞(福島11R)で新プロンプトをテストしたところ、軸=1番人気(実績十分)×相手=6番人気(福島同コース実績あり、12.9倍)というまさに狙い通りの「軸×中穴」の組み合わせが出力され、過去走データも実際の実績(中山金杯勝ち等)を具体的根拠として使っており、質が大きく向上したことを確認済み。** 唯一の観測: S評価が「必ず2頭」の指示に反して3頭付与されるケースが1回あった(軽微、様子見)。
   - **⚠️ 仕様書のうち未実装の部分(ユーザーに要確認・要相談):**
     - **2.5節「予想精度の評価基準」**: レース結果が出た後に「許容できる外れ/許容できない外れ」を判定する事後レビュー基準。プロンプトではなく、`race_recommendation_results`を人間または別セッションが定期的にレビューする際の物差しとして運用する想定。まだ仕組み化していない
     - **6節「データ取得の優先順位」**: Supabase MCP+netkeiba個別URL(`shutuba_past.html`等)経由の手動リサーチ手順が書かれているが、これは**チャットでの手動診断ワークフロー**を指しており、自動化された`/api/races/[raceId]/diagnose`ルート(Vercel上で動く、ライブでnetkeibaを叩かない設計)とは別物。混同しないよう区別が必要
     - **7節「週間運用フロー」**: 週中の軽い診断→金土に確定オッズで本気診断、という二段階運用。現状は「再診断する」ボタンで同じ仕組みを手動的に代替できるが、専用の二段階フローは未実装
     - **8節「診断表フォーマット」**: 個別馬を「能力」「妙味」の2列に分けて表示する案(現状はhorse_rank一本のS/A/B/C)。UIとスキーマの両方に変更が要るため、着手前に相談したい
     - **9節「モデル運用」**: 全体分析を先に整理してからプロンプトに含める二段階LLM呼び出し。実装するとAPI課金がほぼ倍になるため、着手前に相談したい
