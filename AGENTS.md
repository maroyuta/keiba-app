...
**次回セッションが最初にやるべきこと:**
1. **07-11のodds_winバックフィルが完了したか確認。**
   ```
   npm run sync:netkeiba:payouts -- --date 2026-07-11 --env-file .env.local
   ```
   (再実行しても冪等、payoutsもrace_entriesもupsert/updateなので害はない)。完了したら:
   ```sql
   select count(*) from race_entries re join races r on r.id=re.race_id
   where r.race_date='2026-07-11' and re.odds_win > 0;
   ```
   で0件超になっているか確認する。
2. **odds_winが埋まっていたら、07-11の該当14レース(honmei設定済みだがbet_amount未設定だったもの)を
