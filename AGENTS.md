...
**次回セッションが最初にやるべきこと:**
1. **✅完了(セッション終了直前に確認済み): 07-11のodds_winバックフィルが成功した。** 476頭中463頭に
   実際のオッズが入った(残り13頭は出走取消等でnetkeiba側にもオッズが無いと推測、実害なし)。
   `npm run sync:netkeiba:payouts -- --date 2026-07-11 --env-file .env.local`は再実行しても冪等
   (payoutsもrace_entriesもupsert/update)なので、他の日付にも同じ手順を使い回せる。
2. **odds_winが埋まったので、07-11の該当14レース(honmei設定済みだがbet_amount未設定だったもの)を
   再診断する。** 再診断は追加課金が発生する(約14件×$0.16≈$2.2、standardのみ再実行すればよく
   screeningからやり直す必要はない、ただし現状の`/api/races/[id]/diagnose`はscreening→standardを
   まとめて1回で呼ぶ設計なので、実際には通常通りPOSTし直す形になる)。**必ずユーザーに実行前に
   金額を確認すること**(このセッション終盤、ユーザーが「¥1,000くらいに納めて」と予算を明示していた)。
3. 再診断後 `python3 scripts/compute_recommendation_results.py --env-file .env.local` で
   回収率を再集計し、n=27→n=40程度に拡大した結果と、新しい軸選定ルール(人気寄せ修正)適用後の
   軸の複勝率が改善したかを確認する。

**セッション終了時点の実測コスト(このプロジェクト全期間の`api_usage_log`合計): 約$7.40(≈¥1,111)。**
