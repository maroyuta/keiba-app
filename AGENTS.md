...
**セッション終了時点の実測コスト(このプロジェクト全期間の`api_usage_log`合計): 約$7.40〜(14レース
再診断分が積み増し中、正確な最終額は次回確認)。** ユーザーは$20チャージ済みだが、「¥1,000くらいに
納めて」という節度ある予算感覚を持っているため、今後もまとまった額を使う前に確認を取ること。
