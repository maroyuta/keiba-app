...
**「今日実際に開催されるレースの診断が出せるか」は、レース前オッズが解決したことで体感50〜60%程度まで改善。** 2026-07-12の七夕賞での実地テスト時点(過去走データ・オッズ両方が空)ではSonnetを強制実行しても全16頭C評価・買い目なしだったが(詳細は下記「⏸️ 引き継ぎ中の相談」参照)、その後オッズは`fetch_odds.py`で解決した。**残る唯一のデータギャップは過去走データ(`past_performances`)の全レース投入・自動化のみ**(スクレイパー自体は検証済み、スケジュール化がまだ)。これが埋まれば実際のレースで実用的な診断が出せる状態になる見込み。
...
5. **JV-Link接続 — ✅ 疎通確認完了 (2026-07-11)。** Windows PC側でClaude Codeを使い、実際にJV-Linkから生データの取得に成功した(RA=レース詳細37件、SE=馬毎レース情報492件、JG=1002件、合計1531件を取得・確認済み)。作業はこのリポジトリではなくWindows PC上の`C:\Users\maroy\OneDrive\デスクトップ\jvlink\`で別途行われており、このリポジトリの`scripts/jvlink/fetch_raw.py`(Mac側セッションで作成した雛形)とは別の実装になっている。Windows側での実装・デバッグ過程で判明した重要な知見:
   ...
   - **次回最優先: (1)** 実際に1週間分放置してみて`run_weekly_sync.py`/`ComputeRecommendationResults`の自動実行が無人で回るか確認する。**(2)** 実際の診断結果が確定済みレースに出た後、本当に的中したケースでrace_recommendation_resultsの値がnetkeiba等の実結果と一致するかend-to-endで検証する
