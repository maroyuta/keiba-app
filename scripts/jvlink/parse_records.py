...
def parse_slop(data: bytes) -> dict:
    ...
    先頭34バイト(tc(1)+調教年月日時刻(8+4)+血統登録番号(10))は実データ64,357件で
    統計検証済み(2026-08-05): 調教時刻が終日4〜9時台に収まる(トレセン早朝調教の実態と
    完全一致)、血統登録番号の生年部分が2015〜2023の実在範囲に収まる、など。
    **1バイト目の"tc"はJV-Data仕様書上「トレセン区分(美浦/栗東)」とされているが、
    JRA公式サイト/JBISで確認した実馬2頭の所属と付き合わせても一致しなかった
    (同じtc=0の馬に美浦・栗東どちらも実在)。何を表す値かは不明のため、raw値のみ保持し
    施設(facility)としては使わない(load_to_supabase.py側でNoneのまま)。**

    残り24バイト(4/3/2/1ハロンの合計タイムとラップタイム)は、netkeibaで実際に公表されている
    ロードトレイル(2026-07-29 栗坂調教、掲載値54.2-39.1-25.2-12.7)と生バイトを直接突き合わせて
    確定した(2026-08-05)。4バイトの累計タイムと3バイトのラップタイムが交互に並ぶ構造:
    ...
    「累計 − ラップ = 次の累計」の整合性を全64,357件で検算し99.97%が一致(誤差0.1秒以内)。
    いずれも4桁/3桁の数値をそのまま10で割った秒数(他のJV-Dataの時間系項目と同じ/10スケール)。
    """
    c = ByteCursor(data)
    d = {}
    parse_record_id(c, d)
    ...
    d["ketto_num"] = c.take(10)
    d["cum_800"] = c.take(4)  # 4F(800m)地点までの累計タイム
    d["lap_800_600"] = c.take(3)  # 800m→600mのラップ
    d["cum_600"] = c.take(4)  # 3F(600m)地点までの累計タイム
    d["lap_600_400"] = c.take(3)  # 600m→400mのラップ
    d["cum_400"] = c.take(4)  # 2F(400m)地点までの累計タイム
    d["lap_400_200"] = c.take(3)  # 400m→200mのラップ
    d["cum_200"] = c.take(3)  # 1F(200m)地点までの累計タイム(=最終200mのラップそのもの)
    return d
