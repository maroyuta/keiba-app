...
## parse_slop (坂路調教情報HC、2026-08-05追加、部分実装)

`fetch_raw.py "SLOP" <fromtime> 1 <outdir>`で取得できるHC(坂路調教)レコード(58バイト固定長)を
実データ64,357件(2025年7〜9月分)から統計的に逆算した。**レコードヘッダ〜血統登録番号
(先頭34バイト)は高信頼**(調教時刻が全件4〜9時台に収まる=トレセン早朝調教の実態と一致、
血統登録番号の生年部分が2015〜2023の実在範囲、等で検証済み)。**残り24バイト
(「4/3/2/1ハロンの合計タイム・ラップタイム」に相当するはずの区間)は未確定のまま**——
単調減少チェックで最良でも69.8%の整合率にとどまり、公式仕様書(JVData_Struct.cs相当)を
確認できなかったため、`time_fields_raw`として生の文字列のみCSVに保持し、意味付けはしていない。
また1バイト目の`tc`(仕様書上は「トレセン区分・美浦/栗東」)も、`horses.trainer_affiliation`
(netkeiba由来の実データ)と突き合わせたところ一致せず、何を表す値か不明のため使っていない。

`load_to_supabase.py --hc-csv`は、この結果**調教年月日時刻・血統登録番号(→horse_id)のみ**を
`training_sessions`へupsertする(`facility`はnull、`lap_times_sec`は空オブジェクト`{}`)。
「直近いつ調教したか」というrecencyシグナルとしては使えるが、区間タイムによる自己ベース比較
(prompts.tsのPEDIGREE_TRAINING_RULESが想定する使い方)はまだできない。実データで
38,074件(3,712頭分、2025-07-22〜2025-09-11)の初回バックフィル済み。区間タイムを実装するには
JRA-VAN公式のJV-Data仕様書でバイト位置を確認すること。

`run_weekly_sync.py`に`SLOP`のfetch→parse→load(`last_sync_slop.txt`で差分管理、初回は直近7日分)
を追加済み。BLOD(血統)と同じくoption=2の軽量取得には含まれないためoption=1を使う。
