...
## parse_slop (坂路調教情報HC、2026-08-05追加)

`fetch_raw.py "SLOP" <fromtime> 1 <outdir>`で取得できるHC(坂路調教)レコード(58バイト固定長)を
実データから統計的に逆算し、netkeiba公表値との突き合わせで確定した。

**レコードヘッダ〜血統登録番号(先頭34バイト)** は当初から高信頼(調教時刻が全件4〜9時台に収まる=
トレセン早朝調教の実態と一致、血統登録番号の生年部分が2015〜2023の実在範囲、等で検証済み)。
**1バイト目の`tc`(仕様書上は「トレセン区分・美浦/栗東」)は、JRA公式サイト/JBISで確認した実馬
(ハーツシンフォニー=美浦、プライベートアイズ=栗東)がどちらも`tc=0`だったため、facilityとしては
使っていない**(何を表す値かは不明、`facility`列は常にnull)。

**残り24バイト(区間タイム)は2026-08-05、netkeiba premiumで公表されているロードトレイルの
実際の調教タイム(2026-07-29、栗東坂路、掲載値「54.2-39.1-25.2-12.7」)と生バイト列を直接
突き合わせて確定した。** 4バイトの累計タイムと3バイトのラップタイムが交互に並ぶ構造:
`[4F(800m)累計:4B][800-600mラップ:3B][3F(600m)累計:4B][600-400mラップ:3B]`
`[2F(400m)累計:4B][400-200mラップ:3B][1F(200m)累計=最終ラップ:3B]` = 4+3+4+3+4+3+3 = 24バイト。
「累計 − ラップ = 次の累計」の整合性を実データ64,357件全件で検算し99.97%が一致(閾値0.1秒)。
`parse_records.py`の`parse_slop`が`cum_800`/`lap_800_600`/`cum_600`/…の各フィールドに分解し、
`load_to_supabase.py --hc-csv`が`lap_times_sec`(`{"800":..,"600":..,"400":..,"200":..}`)・
`total_time_sec`(=cum_800)として`training_sessions`へ書き込む。`facility`は上記の理由でnullのまま。

実データで初回バックフィル済み(2025-07-22〜2025-09-11の38,074件+直近週7,848件のうち対象馬分)。
`run_weekly_sync.py`に`SLOP`のfetch→parse→load(`last_sync_slop.txt`で差分管理、初回は直近7日分)を
追加済み。BLOD(血統)と同じくoption=2の軽量取得には含まれないためoption=1を使う。

**WOOD(ウッドチップ調教、WC)は未着手。** `fetch_raw.py "WOOD" ...`が2026-08-05時点で実機上
ハングする現象を確認(CPU使用率ほぼ0のままプロセスが終わらない、1時間範囲・1日範囲のどちらでも
再現)。原因未調査。SLOPは同条件で問題なく取得できているため、WOOD固有の問題の可能性が高い。
WOODも同じ4+3+4+3+4+3+3構造を試す価値がある(坂路とウッドチップでレコード仕様が近いため)。
